...
import { execCase } from '../lib/executor.js';
import { runSubmissionInBackground } from '../lib/submit-runner.js';
...
problemsRouter.post('/:slug/submit', authRequired, async (req, res, next) => {
  try {
    ...
    runSubmissionInBackground(submission.id, {
      languageId,
      sourceCode,
